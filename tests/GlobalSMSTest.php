...
namespace Dominik\GlobalSMS\Tests;
...
/**
 * Class GlobalSMSTest
 * @package Dominik\GlobalSMS\Tests
 */
class GlobalSMSTest extends \PHPUnit_Framework_TestCase
{
     ...
     * @test
     */
    public function it_should_send_getSmsResponses_request()
    {
        $result = 'result';
        $mock = $this->mockSoapClient();
        $mock->shouldReceive('getSmsResponses')->once()->andReturn($mock);
        $mock->getSmsResponsesResult = $result;
        $service = $this->getService();
        $service->setSoapClient($mock);
        
        $this->assertEquals($result, $service->getSmsResponses());
    }
